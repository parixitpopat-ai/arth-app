...
function AppContent({ onLock }) {
  ...
  const getCardSummary = useCallback((card)=>{
    ...
    // Payments AND refunds after statement date reduce the outstanding
    const paymentsSinceStatement = txns.reduce((sum,t)=>{
                  ...
                  {useItemizedLines&&lineItems.length>0&&(
                    <div style={{ marginTop:8,display:"flex",flexDirection:"column",gap:6 }}>
                      ...
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:4 }}>
                        ...
                        <span style={{ color:Math.abs(lineItemsTotal-amt)<0.01?T.success:T.warn,fontSize:13,fontWeight:800 }}>{sym}{fmt(lineItemsTotal)}{Math.abs(lineItemsTotal-amt)>=0.01?" (mismatch)":""}</span>
                      </div>
                    </div>
                  )}
                </div>
  ...
  const EditBillModal = ({ b, onClose }) => {
    ...
    const [billerCategory,setBillerCategory]=useState(b.billerCategory||"");
    const [consumerNumber,setConsumerNumber]=useState(b.consumerNumber||"");
    const [lastPaidAmount,setLastPaidAmount]=useState(b.lastPaidAmount?String(b.lastPaidAmount):"");
    const [lastPaidDate,setLastPaidDate]=useState(b.lastPaidDate||"");
    const [showBillerDetails,setShowBillerDetails]=useState(!!(b.billerCategory||b.consumerNumber));
    const [billingModel,setBillingModel]=useState(b.billingModel||"prorata");
    const [activationDate,setActivationDate]=useState(b.activationDate||"");
    const [validFrom,setValidFrom]=useState(b.validFrom||"");
    const [validUntil,setValidUntil]=useState(b.validUntil||"");
    const [validityDays,setValidityDays]=useState(b.validityDays?String(b.validityDays):"");
    const [periodStart,setPeriodStart]=useState(b.periodStart||"");
    const [periodEnd,setPeriodEnd]=useState(b.periodEnd||"");
    const [membershipEndDate,setMembershipEndDate]=useState(b.membershipEndDate||"");
    const [freeTrialEndDate,setFreeTrialEndDate]=useState(b.freeTrialEndDate||"");
    const [isUsageBased,setIsUsageBased]=useState(b.isUsageBased||false);
    ...
    const save=()=>{
      ...
      setBills(prev=>prev.map(x=>x.id===b.id?{...x,name:name.trim(),amount:parseFloat(amount)||0,billDate:billDate||x.billDate||todayStr(),dueDate,catId,subId:subId||null,recurring,frequency,merchant:merchant.trim()||name.trim(),invoiceNo:invoiceNo.trim(),imageBase64:editPhoto,splitPeople:peopleSplit,groupId:editGroup||null,groupCollectiveAmount,myShare,billerCategory:billerCategory||null,consumerNumber:consumerNumber.trim()||null,lastPaidAmount:lastPaidAmount?parseFloat(lastPaidAmount):null,lastPaidDate:lastPaidDate||null,billingModel:billingModel||"prorata",activationDate:activationDate||null,validFrom:validFrom||null,validUntil:validUntil||null,validityDays:validityDays?Number(validityDays):null,periodStart:billingModel==="calendar"?periodStart||null:null,periodEnd:billingModel==="calendar"?periodEnd||null:null,membershipEndDate:membershipEndDate||null,freeTrialEndDate:freeTrialEndDate||null,isUsageBased,autoGenerate}:x));
      onClose();
    };
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
            <div style={{ background:T.input,borderRadius:12,padding:"10px 12px" }}>
              ...
              {billerAccounts.length===0
                ...
                : <select style={inp} value={billerAccountId} onChange={e=>{ const id=e.target.value; setBillerAccountId(id); if(id){ const ba=billerAccounts.find(b=>b.id===id); if(ba){ setName(ba.name); setMerchant(ba.provider||ba.name); setBillerCategory(ba.type||""); setConsumerNumber(ba.consumerNo||""); } } }}>
