...
*relationships*, not merely contact records — the framing that PG001
surfaced while designing it.
...
Household Budget. They do not own independent budgets. Confirmed
consistent with ADR-024 — Monthly Planning Budget is the same pool as
Safe to Spend's Budget, not a second one. `Person.spendBudget` and
`Group.manualLimit` are allocations *of* that one pool, not separate
pools of their own.

**Rule 3.** Transactions are the single source of truth. People,
Groups, Budget, Forecast, and Insights all derive their financial state
from tagged transactions rather than maintaining separate ledgers.
**Verified true today, not aspirational:** `settlements` (the
`owesMe`/`iOwe` figures) is a `useMemo` — computed fresh from
transaction data on every render, confirmed by checking the code, not
a separately-maintained running total that could drift out of sync.
...
**Purpose:** freeze the distinction between two genuinely different
questions that were previously conflated into one "Safe to Spend"
number — confirmed as a real bug (not a labeling preference) by
checking how `monthSpend` already works: once a bill is paid, it
becomes an expense transaction and is *already* counted against
Budget. A second number that also pre-emptively subtracted unpaid
bills was answering a different question while wearing the same name.
...
4. **Protected Money is a first-class concept** — money already committed that Outlook is explicitly telling the user not to spend on something else.
5. Insurance Premium remains `Bill.type` — ADR-021/023 unchanged, explicitly confirmed not reversed. It flows into Protected Money like any other Bill.

**What this does NOT do:** does not introduce new Bill sub-types, does not add new fields to Bill, does not touch the Forecast Engine's pure functions (`calculateProjectedBalance` etc. remain untouched) — this ADR is entirely about which existing/newly-added calculation feeds which UI concept, not new domain modeling.
