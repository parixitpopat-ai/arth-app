...
function AppContent({ onLock }) {
  ...
  const People = () => {
    ...
    if(selectedGroup){
      ...
      const overallPaidByMe = fyTxns.filter(t=>t.type==="expense").reduce((sum,t)=>sum+Number(t.amount||0),0);
              ...
              <div style={{ flex:1 }}>
                ...
                {(()=>{
                  ...
                  const memberBalances = allTileIds.map(pid=>{
                    ...
                    // total spend: for me = all I paid in group (use allocation amount if multi-group)
                    // for others = sum of their attributed amounts
                    const totalSpend = isMe
                      ? gTxns.filter(t=>t.type==="expense").reduce((s,t)=>{
                          const alloc = t.groupAllocations?.find(ga=>ga.groupId===g.id);
                          return s + (alloc ? Number(alloc.amount||0) : Number(t.amount||0));
                        },0)
                      : gTxns.filter(t=>t.type==="expense"&&t.people&&t.people[pid]).reduce((s,t)=>s+Number(t.people[pid]?.amount||0),0);
