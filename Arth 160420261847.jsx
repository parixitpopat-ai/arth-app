...
function AppContent({ onLock }) {
  ...
  const settlements = useMemo(()=>{
    ...
    txns.forEach(t=>{
      ...
      if(t.type==="expense"){
        // forPerson attribution → counts as receivable
        if(t.forPerson && Number(t.tagPersonAmount||0)>0){
        ...
        // tagItems person entries → count as receivable
        (t.tagItems||[]).forEach(item=>{
          if(item.targetType!=="person"||!item.targetId||!Number(item.amount||0)) return;
      ...
      <div onClick={e=>e.target===e.currentTarget&&setSettleTxn(null)} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200 }}>
        <div style={{ background:T.card,borderRadius:"22px 22px 0 0",padding:"20px 18px 40px",width:"100%",maxWidth:430,maxHeight:"90vh",overflowY:"auto" }}>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <button onClick={()=>setShowSettleSms(v=>!v)} style={{ background:"none",border:"none",color:T.info,fontSize:11,fontWeight:700,cursor:"pointer",padding:"4px 0",fontFamily:"Nunito,sans-serif" }}>📱 {showSettleSms?"Hide":"Paste bank SMS to auto-fill ref"}</button>
            {showSettleSms&&<textarea style={{ ...inp,height:64,resize:"none",marginTop:4,fontSize:11 }} placeholder="Paste bank SMS here…" value={settleSms} onChange={e=>{ setSettleSms(e.target.value); const ref=extractTxnReference(e.target.value); if(ref) setSettleRef(ref); }}/>}
          </div>
  ...
  const People = () => {
    ...
    if(selectedPerson){
      ...
      const taggedTxns=(p.isMe
        ? txns.filter(t=>{
            if(t.type!=="expense") return false;
            return (t.forPerson===p.id || Boolean(t.people?.__me__)) && personMonthFilter(t);
          })
        ...
        <div style={{ padding:"14px 16px 0" }}>
          ...
          <div style={card}>
            <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:14 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                {(()=>{
                  ...
                  const allTileIds = [meP.id, ...memberIds.filter(pid=>pid!==meP.id)];
                  const memberBalances = allTileIds.map(pid=>{
                    const p = pid===meP.id ? meP : getPerson(pid);
                    if(!p) return null;
                    const isMe = pid===meP.id || p.isMe;
                    const owesMe = isMe ? 0 : getGroupMemberOwed(g.id, pid);
                    // total spend: for me = group's allocated amount
                    // for others: check t.people[pid] (person-level split) first,
                    // then fall back to their share of group allocation if member count known
                    const totalSpend = isMe
                      ? gTxns.filter(t=>t.type==="expense").reduce((s,t)=>s+getGroupTxnAmt(t,g.id),0)
                      : gTxns.filter(t=>t.type==="expense").reduce((s,t)=>{
                          // Direct person split in t.people
                          if(t.people?.[pid]) return s + Number(t.people[pid]?.amount||0);
                          // Group collective — divide equally among members if no individual split
                          const groupAlloc = t.groupAllocations?.find(ga=>ga.groupId===g.id);
                          if(groupAlloc && groupAlloc.mode==="owes") {
                            const memberCount = Math.max(1, (g.members||[]).length);
                            return s + Number(groupAlloc.amount||0) / memberCount;
                          }
                          return s;
                        },0);
                    return { p, owesMe, totalSpend, isMe };
                  }).filter(Boolean);
                  const anyData = memberBalances.some(m=>m.totalSpend>0||m.owesMe>0);
                  if(!anyData) return null;
                  return (
                    <div style={{ marginTop:10 }}>
                      <div style={{ color:T.sub,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:6 }}>Individual Spend</div>
                      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
                        {memberBalances.map(({p,owesMe,totalSpend,isMe})=>(
                          <div key={p.id} style={{ background:T.input,borderRadius:10,padding:"10px 10px",position:"relative" }}>
                            ...
                            {owesMe>0&&<div style={{ color:T.success,fontSize:9,fontWeight:700,marginTop:2 }}>owes {sym}{fmt(owesMe)}</div>}
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div style={{ borderTop:`1px solid ${T.border}`,paddingTop:12,marginTop:4 }}>
              ...
              {(()=>{
                const displayTxns = groupViewMode==="overall"
                  ? gTxns.slice().sort((a,b)=>(b.date||"").localeCompare(a.date||""))
                  : mTxns;
                return <>
                  <div style={{ color:T.sub,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:10 }}>Expenses — {groupViewMode==="overall"?"All time":smLabel}</div>
                  {displayTxns.length===0
                    ?<div style={{ color:T.sub,fontSize:12,textAlign:"center",padding:20 }}>No expenses{groupViewMode==="monthly"?` for ${smLabel}`:""}</div>
