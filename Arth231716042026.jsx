            ...
            <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:14 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                {(()=>{
                  ...
                  // Only show Individual Spend when persons are directly tagged in t.people
                  // (not for group-level collective allocations)
                  const hasPersonLevelSplit = gTxns.some(t=>
                    t.type==="expense" &&
                    memberIds.some(pid=>t.people?.[pid] && Number(t.people[pid]?.amount||0)>0)
                  );
                  if(!hasPersonLevelSplit) return null;
                  const memberBalances = allTileIds.map(pid=>{
                    ...
                    // Only count direct person-level splits (not group collective)
                    const totalSpend = isMe
                      ? gTxns.filter(t=>t.type==="expense").reduce((s,t)=>s+getGroupTxnAmt(t,g.id),0)
                      : gTxns.filter(t=>t.type==="expense"&&t.people?.[pid]).reduce((s,t)=>s+Number(t.people[pid]?.amount||0),0);
