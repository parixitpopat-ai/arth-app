...
**Schema change:** new `schools[]` collection (id, name, address?). ⚠️ **CORRECTED (post-WP-C1/C2, reflects the actually-approved architecture, not the originally-drafted one):** School relationship records use `schoolRelationships[] → { id, schoolId, personId, status, statusHistory, createdAt }` — the same shape WP-C2 actually built and shipped on `origin/main` (`e3f6900`). There is **no** `targetType`/`targetId` — WP-C1 Step 1 succeeded (School reuses `lifecycle.js`'s generic engine via its own sibling persisted shape), so the generalized-migration path (Step 2, which would have introduced `targetType`/`targetId` on `membershipRelationships[]`) was never needed and was never built. `membershipRelationships[]`'s own persisted shape remains completely untouched — no Membership schema migration occurred or is planned. This correction brings the document in line with reality; it does not authorize or request any change to the already-shipped implementation.
