...
| **Multi-dimensional transaction contribution** | Missing entirely | Every transaction today carries a single category attribution and a single person/group attribution — there's no mechanism for one transaction to feed more than one budget dimension at once. (Note: an earlier draft of this row cited "ADR-030" for the frozen transaction shape; that document was part of an orphaned pre-repository sandbox set, never canonical — see CR-ACC-BUD-001 for the real, repository-verified attribution rule.) |
...
- **Transaction Contribution** (or Allocation Link) — the mechanism by which one transaction can count toward more than one Budget Allocation at once. This has no analogue in the current system at all — it's the single largest structural gap, and it intersects Transaction's real attribution shape (`t.people`, resolved per CR-ACC-BUD-001).
...
**Coupling to Transaction's attribution model**
- The target model's core new capability — one transaction contributing to multiple budget dimensions — directly intersects Transaction's real attribution shape (`t.people`). At the time this document was originally drafted, that shape's reconciliation was believed open, citing "ADR-030" — a document later confirmed (via the repository audit trail, see EDL-001) to have been part of an orphaned pre-repository sandbox set that was never committed to the real repo and is not canonical governance. **This has since been resolved directly against real repository evidence**, not against that orphaned document — see `CR-ACC-BUD-001`, which confirms `mode:"spent_on"` is attributable spend and `mode:"owes"` is excluded, and registers the rule in `CBR-TRX-person-attribution-semantics.md`. BUD-000's biggest structural gap is therefore no longer coupled to an open Person-attribution question. It remains coupled to Category-dimension Attribution's design, which CR-ACC-BUD-001 leaves as a recommended-but-undecided path — that dependency stands as originally noted, just narrowed to the one dimension still actually open.
