...
|---|---|---|---|---|---|---|---|
| CR-001 | Duplicate | Settlement allocation — 4 locations (`applyRepaymentAllocations`, `SettleModal.settle()`, `AddModal` L4480, `AddModal` L4641) | Transactions domain canonical settlement service | ADR-032 | **In Progress** — plain-transaction path canonical in both functions (TRX-002C3/C4a). `applyRepaymentAllocations`'s bill-kind branch now also canonical (TRX-002C4b), via `settlePersonShareOnBill` reusing the shared `TransactionPersonShare` value object per AQ-002 — does not require the full Bill/Payable aggregate. **Still legacy:** `SettleModal.settle()`'s bill-kind branch, `applyRepaymentAllocations`'s group-txn/group-bill/tagged branches, and `AddModal` L4480/L4641 entirely. Not Complete until every location and branch is repointed and the full suite stays green. | — | TRX-002C3, TRX-002C4a, TRX-002C4b (all partial) |
