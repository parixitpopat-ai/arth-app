...
**Decision:** Built `SettlementService` (stateless orchestrator per ADR-033) with 5 passing tests, proven polymorphic against the `SettlementTarget` contract using `Transaction` instances, matching the real observed "excess kept as advance" allocation behavior found during this session's debugging. Explicitly did NOT proceed to repoint or remove `applyRepaymentAllocations`/`SettleModal.settle()` in `App.jsx` — checked the stated success criteria ("no regression tests fail") against reality and found no regression tests exist for that code at all (confirmed, ARCH-001). Recommended characterization tests as a prerequisite before any production retirement, rather than verify "no behavior change" by inspection alone on the exact code already responsible for 3 confirmed bugs from being inspected-not-tested.

**Reasoning:** Proceeding to delete/repoint production settlement code without a mechanical safety net would repeat the precise failure mode this entire session exists to correct — "we read it carefully" was never sufficient evidence anywhere else in this process, and settlement code is the highest-stakes place to apply that standard, not the place to relax it under time pressure.

**What it doesn't decide:** Whether characterization tests become their own ticket or a lighter manual verification suffices — explicitly left as your call, since it trades rigor for speed and is a product-scale decision, not an architectural one.

**Affected Tickets:** TRX-002C (partial), prerequisite ticket (not yet opened, pending your decision)
**Affected Modules:** Transactions, Settlements
**Affected ADRs:** ADR-033 (SettlementService implemented)
...
**Decision:** Built and ran (not just designed) TRX-002B's full scope, in the exact order specified: `Money` (value object, centralizes non-negative/rounding invariants), `LineItem` (child entity, deliberately no amount-reconciliation enforcement per Team 1's Open invariant), `TransactionPersonShare` (implements Team 1's two core invariants structurally — `settled` computed, `settledAmt` clamped, immutable), `Transaction` aggregate root (`post`/`edit`/`delete`/`applySettlement`, `SettlementTarget` contract per ADR-033, constructor invariants for type/date/accountId/personShares-only-on-expense). Repository integration via `PostTransactionHandler`/`ApplySettlementHandler` on TRX-002A's existing `CommandDispatcher` — no new mechanism. 38 new tests (76 total combined with TRX-002A), every one of Team 1's invariants and the CBR's "no duplicate settlement logic" claim made executable, all run and passing (42/42 combined suite).

**Reasoning:** Following the stated order (skeleton → behavior → events → repository → tests) meant each layer was verifiable before the next was built. Explicitly did not touch `SettlementService`/`Payable`/any `App.jsx` code — those are TRX-002C/D's scope, and building them now would be exactly the feature-expansion this ticket was scoped to avoid.

**What it doesn't decide:** CR-001's actual migration (repointing `applyRepaymentAllocations`/`SettleModal.settle()` to this new `applySettlement()`) — the canonical implementation now exists, but the CBR stays at 8/4 until the legacy duplicates are actually retired, which is TRX-002C's job.

**Affected Tickets:** TRX-002B (done), TRX-002C (next)
**Affected Modules:** Transactions
**Affected ADRs:** ADR-033 (SettlementTarget contract implemented), ADR-018 (permanent delete implemented), ADR-017/030 (type taxonomy enforced)
