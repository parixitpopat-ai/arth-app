...
|---|---|---|---|---|---|---|---|
| CR-001 | Duplicate | Settlement allocation — 4 locations (`applyRepaymentAllocations`, `SettleModal.settle()`, `AddModal` L4480, `AddModal` L4641) | Transactions domain canonical settlement service | ADR-032 | **In Progress** — Step 1 done: `applyRepaymentAllocations`'s txn-kind person-share branch repointed to `Transaction.applySettlement()` via a proven-equivalent adapter (TRX-002C3). Step 2 (`SettleModal.settle()`) and the remaining branches (bill-kind, group-txn, group-bill, tagged) still on legacy logic. Not Complete until all steps land and characterization suite stays green. | — | TRX-002C3 (partial) |
